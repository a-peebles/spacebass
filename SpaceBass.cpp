...
#pragma clang diagnostic ingored "-Wmain"
...
/**
When an audio preference is changed.
Update our voice manager with new sample rate.
*/
...
void SpaceBass::ProcessDoubleReplacing(double** inputs, double** outputs, int nFrames)
{
  // Mutex is already locked for us.
	...
	for (int i = 0; i < nFrames; ++i) {
		// update values of MIDI receiver
		mMIDIReceiver.advance();
		// gets the value played
		leftOutput[i] = rightOutput[i] = voiceManager.nextSample();
	}
	// Moves the MIDI queues front
	mMIDIReceiver.Flush(nFrames);
}
/**
Called whenever application receives a midi message.
Uses SetDirty so renderer will draw on next paint cycle.
*/
...
/**
Reacts to key presses on the virtual keyboard.
*/
void SpaceBass::processVirtualKeyboard() {
	...
	if (lastVirtualKeyboardNoteNumber >= virtualKeyboardMinimumNoteNumber && virtualKeyboardNoteNumber != lastVirtualKeyboardNoteNumber) {
		// The note number has changed from a valid key to something else, release key.
		IMidiMsg midiMessage;
		midiMessage.MakeNoteOffMsg(virtualKeyboardMinimumNoteNumber, 0);
	...
	if (virtualKeyboardNoteNumber >= virtualKeyboardMinimumNoteNumber && virtualKeyboardNoteNumber != lastVirtualKeyboardNoteNumber) {
		// A valid key is pressed that wasn't pressed before, sends "note on message to MIDI receiver
		IMidiMsg midiMessage;
		midiMessage.MakeNoteOnMsg(virtualKeyboardMinimumNoteNumber, virtualKeyboard->GetVelocity(), 0);
		mMIDIReceiver.onMessageReceived(&midiMessage);
	}
...
void SpaceBass::CreateGraphics() {
	// Creates a window and attaches background and key images.
	IGraphics* pGraphics = MakeGraphics(this, kWidth, kHeight);
	pGraphics->AttachBackground(BG_ID, BG_FN);
	IBitmap whiteKeyImage = pGraphics->LoadIBitmap(WHITE_KEY_ID, WHITE_KEY_FN, 6);
	IBitmap blackKeyImage = pGraphics->LoadIBitmap(BLACK_KEY_ID, BLACK_KEY_FN);
	//Coordinates for the different keys
	//							  C#	 D#			 F#		 G#		 A#
	int keyCoordinates[12] = { 0, 10, 17, 30, 35, 52, 61, 68, 79, 85, 97, 102 };

	// Attaches key coordinates to their keys on the on screen keyboard controller
	...
	// Attaches control to the window
	pGraphics->AttachControl(mVirtualKeyboard);
	// Loads the bitmaps for selection
